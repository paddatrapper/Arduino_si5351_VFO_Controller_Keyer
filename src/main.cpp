...
void setup() {
    ...
    pinMode(PIN_PUSHBTTN_REAR, INPUT);


    for (byte i = 0; i < 8 ; i++) {
        filterPCF.write(i, 0); //turn all the filter relays off
    }
...
void loop() {
    ...
    switch (state) {
        case STATE_Rx:
            step_receive(vfos[active_preset]);
            break;
        case STATE_Cw:
            {
                ...
                while (delay > 0) {
                    ...
                    if (new_delay == 0) delay = 0;
                    else if (new_delay == -1) delay -= millis() - start;
